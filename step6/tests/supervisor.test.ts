...
describe("Supervisor step3 phase engine", () => {
  it("creates planning/architecture/design artifacts and succeeds", async () => {
    ...
    expect(createdArtifacts).toHaveLength(6);
    expect(createdArtifacts.map((event) => event.role)).toEqual(["planner", "architect", "designer", "test", "reviewer", "packager"]);
    ...
    expect(artifacts.getAll(sessionId)).toHaveLength(6);
