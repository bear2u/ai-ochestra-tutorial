...
const createGoalValidation = (sessionId: string, iteration: number, passed: boolean): GoalValidationArtifact => ({
  id: `goal-${sessionId}-${iteration}`,
  sessionId,
  phase: "goal_validation",
  iteration,
  passed,
  summary: passed ? "goals ok" : "goals missing",
  checks: [
    {
      id: "dir:example",
      label: "Directory exists: example",
      passed,
      detail: passed ? "exists" : "missing"
    }
  ],
  missingTargets: passed ? [] : ["example"],
  suggestions: passed ? [] : ["Create example directory"],
  createdAt: new Date().toISOString()
});
...
describe("ArtifactStore", () => {
  ...
  it("returns refs and all artifacts in phase order", () => {
    ...
    const goalValidation1 = createGoalValidation("session-1", 1, false);
    const goalValidation2 = createGoalValidation("session-1", 2, true);
    ...
    store.save("session-1", plan);
    store.save("session-1", architecture);
    store.save("session-1", goalValidation1);
    store.save("session-1", goalValidation2);
    ...
    expect(store.getRefs("session-1")).toEqual({
      ...
      design: design.id,
      goal_validation: goalValidation2.id,
    ...
    expect(store.getGoalValidationArtifacts("session-1").map((artifact) => artifact.iteration)).toEqual([1, 2]);
