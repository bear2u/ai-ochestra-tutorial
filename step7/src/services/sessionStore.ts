...
export class SessionStore {
  ...
  private static readonly allChannel = "session:*";

  create(input: SessionInput): SessionState {
    const startedAt = new Date().toISOString();
    const maxIterations = input.maxIterations ?? input.maxAttempts;
    const maxMinutes = input.maxMinutes;

    const budget: BudgetState | undefined =
      typeof maxIterations === "number" && typeof maxMinutes === "number"
        ? {
            maxIterations,
            maxMinutes,
            startedAt,
            deadlineAt: new Date(new Date(startedAt).getTime() + maxMinutes * 60_000).toISOString(),
            elapsedMs: 0,
  ...
  patchInput(sessionId: string, patch: Partial<SessionInput>): void {
    const current = this.sessions.get(sessionId);
    if (!current) return;
    current.input = {
      ...current.input,
  ...
  updateBudget(sessionId: string, budget: BudgetState): void {
    const current = this.sessions.get(sessionId);
    if (!current) return;
  ...
  setArtifactRef(
    sessionId: string,
    phase: "planning" | "architecture" | "design" | "goal_validation" | "validation" | "review" | "packaging",
    artifactId: string
  ): void {
    const current = this.sessions.get(sessionId);
    if (!current) return;
  ...
  pushEvent(
    ...
    this.emitter.emit(SessionStore.allChannel, event);
    return event;
  }
  ...
  subscribeAll(handler: (event: SessionEvent) => void): () => void {
    this.emitter.on(SessionStore.allChannel, handler);
    return () => this.emitter.off(SessionStore.allChannel, handler);
  }
}
