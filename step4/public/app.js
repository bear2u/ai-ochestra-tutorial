...
const sessionPresets = [
  {
    id: "step4-pipeline-success",
    label: "1) Step4 Pipeline 성공",
    stage: "step4-core",
    description: "pre-loop + patch apply + validation pipeline 성공 흐름을 확인합니다.",
    task: "step4 validation pipeline 전체 성공 시나리오",
    ...
    validationCommands: ["pnpm lint", "pnpm typecheck", "node -e \"console.log('ok'); process.exit(0)\""],
    maxAttempts: 2,
    expected: "planning -> architecture -> design -> implementation/validation -> review/packaging -> session success",
    checkpoints: ["artifact_created 3회", "validation_command_started/completed", "classification 없음(성공)"]
  },
  {
    id: "step4-lint-fail",
    label: "2) Lint 실패 분류",
    stage: "validation-lint",
    description: "첫 단계 lint 실패 시 classification=lint를 확인합니다.",
    task: "step4 lint classification 검증",
    filePaths: ["playground/validation-fail.txt"],
    testCommand: "",
    validationCommands: [
      "node -e \"console.error('lint fail'); process.exit(1)\"",
      "pnpm typecheck",
      "node -e \"console.log('ok'); process.exit(0)\""
    ],
    maxAttempts: 1,
    expected: "validation phase에서 즉시 실패, session failed",
    checkpoints: ["validation_command_failed(첫 커맨드)", "classification lint", "review/packaging skipped"]
  },
  {
    id: "step4-type-fail",
    label: "3) Typecheck 실패 분류",
    stage: "validation-type",
    description: "두 번째 단계 실패 시 classification=type을 확인합니다.",
    task: "step4 type classification 검증",
    filePaths: ["playground/validation-fail.txt"],
    testCommand: "",
    validationCommands: [
      "node -e \"console.log('lint ok'); process.exit(0)\"",
      "node -e \"console.error('type fail'); process.exit(1)\"",
      "node -e \"console.log('ok'); process.exit(0)\""
    ],
    maxAttempts: 1,
    expected: "두 번째 커맨드(typecheck 위치)에서 실패",
    checkpoints: ["validation_command_completed(1)", "validation_command_failed(2)", "classification type"]
  },
  {
    id: "step4-type-fail-retry-loop",
    label: "4) 2단계 실패 재검증 루프",
    stage: "validation-retry-loop",
    description: "2번째 validation command를 실패시켜 Supervisor iteration 재검증 루프를 확인합니다.",
    task: "step4 type 실패 재검증 루프 검증",
    filePaths: ["playground/validation-fail.txt"],
    testCommand: "",
    validationCommands: [
      "node -e \"console.log('lint pass'); process.exit(0)\"",
      "node -e \"console.error('type fail loop'); process.exit(1)\"",
      "node -e \"console.log('test should not run'); process.exit(0)\""
    ],
    maxAttempts: 3,
    expected: "iteration마다 validation 2단계에서 실패하고 maxAttempts 소진 후 session failed",
    checkpoints: [
      "attempt_started(iteration 1..3)",
      "validation_command_failed(stage=type)",
      "tests_failed 반복 후 session_finished(failed)"
    ]
  },
  {
    id: "step4-runtime-fail",
    label: "5) Runtime 실패 분류",
    stage: "validation-runtime",
    description: "timeout/runtime 오류 시 classification=runtime을 확인합니다.",
    task: "step4 runtime classification 검증",
    filePaths: ["playground/validation-fail.txt"],
    testCommand: "",
    validationCommands: ["node -e \"setTimeout(() => {}, 60000)\""],
    maxAttempts: 1,
    expected: "runtime/timeout 오류로 실패",
    checkpoints: ["validation_command_failed", "classification runtime", "session failed"]
  },
  {
    id: "step4-retry-third-success",
    label: "6) 2회 실패 후 3회 성공",
    stage: "validation-retry-success",
    description: "validation을 2번 실패시키고 3번째 iteration에서 성공시켜 Supervisor 재검증 루프를 확인합니다.",
    task: "step4 supervisor 재검증 루프(2 fail -> 3rd pass) 검증",
    filePaths: ["playground/validation-fail.txt"],
    testCommand: "",
    validationCommands: [
      "node -e \"const fs=require('fs');const p='.step4_retry_counter';let n=0;try{n=Number(fs.readFileSync(p,'utf8'))||0}catch{};n+=1;fs.writeFileSync(p,String(n));if(n<3){console.error('forced fail on iteration '+n);process.exit(1)}fs.unlinkSync(p);console.log('pass on iteration '+n);process.exit(0)\""
    ],
    maxAttempts: 3,
    expected: "iteration 1/2에서 tests_failed, iteration 3에서 tests_passed 후 session success",
    checkpoints: [
      "attempt_started(iteration 1..3)",
      "tests_failed x2",
      "tests_passed(iteration 3) -> review/packaging -> session_finished(success)"
    ]
  }
];
...
const defaultPresetHelp = "선택 시 Task, File paths, Test command, Validation commands, Max attempts가 자동으로 채워집니다.";
const defaultPresetPreview = "시나리오를 선택하면 Step4 체크포인트를 미리 보여줍니다.";
