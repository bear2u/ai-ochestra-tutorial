...
describe("Supervisor step3 phase engine", () => {
  it("creates planning/architecture/design artifacts and succeeds", async () => {
    ...
    expect(createdArtifacts).toHaveLength(4);
    expect(createdArtifacts.map((event) => event.role)).toEqual(["planner", "architect", "designer", "test"]);
    ...
    expect(artifacts.getAll(sessionId)).toHaveLength(4);
