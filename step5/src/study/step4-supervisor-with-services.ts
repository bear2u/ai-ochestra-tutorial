...
class MockReviewerAgent implements ReviewerAgentLike {
  async createReview(input: Parameters<ReviewerAgentLike["createReview"]>[0]): Promise<{
    id: string;
    sessionId: string;
    phase: "review";
    iteration: number;
    blockingIssues: { id: string; title: string; detail: string }[];
    nonBlockingIssues: { id: string; title: string; detail: string }[];
    score: number;
    fixPlan: string[];
    createdAt: string;
  }> {
    return {
      id: `review-${input.sessionId}-${input.iteration}`,
      sessionId: input.sessionId,
      phase: "review",
      iteration: input.iteration,
      blockingIssues: [],
      nonBlockingIssues: [{ id: "INFO-1", title: "학습 시나리오 승인", detail: "패키징 진행 가능" }],
      score: 90,
      fixPlan: [],
      createdAt: new Date().toISOString()
    };
