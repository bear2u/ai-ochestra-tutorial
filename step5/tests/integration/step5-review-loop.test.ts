...
describe("step5 review loop integration", () => {
  it("re-enters implementation when review has blocking issues and later succeeds", async () => {
    ...
    const reviewer: ReviewerAgentLike = {
      createReview: async ({ sessionId, iteration }) => ({
        ...
        blockingIssues:
          iteration === 1
            ? [{ id: "BLOCK-1", title: "needs improvement", detail: "iteration 1 intentionally blocked" }]
            : [],
        nonBlockingIssues: [{ id: "INFO-1", title: "review note", detail: "ok" }],
        score: iteration === 1 ? 60 : 92,
        fixPlan: iteration === 1 ? ["apply reviewer fix plan"] : [],
        createdAt: new Date().toISOString()
      })
    };
    ...
    const supervisor = new Supervisor(store, artifacts, workspace, planner, architect, designer, reviewer, dev, test, commandRunner);
    ...
    expect(session.artifactRefs?.review).toBeDefined();

    const reviewArtifacts = artifacts.getReviewArtifacts(sessionId);
    expect(reviewArtifacts).toHaveLength(2);
    expect(reviewArtifacts[0].blockingIssues.length).toBeGreaterThan(0);
    expect(reviewArtifacts[1].blockingIssues).toHaveLength(0);

    const events = store.getEvents(sessionId);
    expect(events.some((event) => event.type === "review_blocking_detected")).toBe(true);
    expect(events.some((event) => event.type === "review_approved")).toBe(true);
  });
});
