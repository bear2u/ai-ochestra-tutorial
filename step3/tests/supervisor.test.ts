...
describe("Supervisor step3 phase engine", () => {
  it("creates planning/architecture/design artifacts and succeeds", async () => {
    ...
    expect(createdArtifacts).toHaveLength(3);
    expect(createdArtifacts.map((event) => event.role)).toEqual(["planner", "architect", "designer"]);
    ...
    expect(artifacts.getAll(sessionId)).toHaveLength(3);
